    ...
    {itinerary.map((item, index) => {
      ...
      return (
        ...
        >
          <button
            onClick={() => toggleAccordion(index)}
            className="w-full px-5 py-4 flex items-start justify-between text-left hover:bg-gray-50 rounded-t-xl"
          >
          ...
          {isOpen && (
            <div className="px-5 pb-5 pt-2 text-gray-600 text-sm md:text-base whitespace-pre-line">
              {item.content}
            </div>
          )}
        </div>
